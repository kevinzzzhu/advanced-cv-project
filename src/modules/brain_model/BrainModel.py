...
class Clipper(torch.nn.Module):
    def __init__(self, clip_variant, clamp_embs=False, norm_embs=False,
        ...
        if clip_variant=="ViT-L/14" and hidden_state:
            ...
            self.tokenizer = CLIPTokenizer.from_pretrained("/opt/data/private/huggingface/stable-diffusion-v1-4", subfolder="tokenizer")
            text_encoder = CLIPTextModel.from_pretrained("/opt/data/private/huggingface/stable-diffusion-v1-4", subfolder="text_encoder").eval()
    ...
    def embed_text(self, text_samples):
        if self.hidden_state:
            clip_text = self.tokenizer(text_samples, padding="max_length", max_length=77, return_tensors="pt", truncation=True).input_ids.to('cuda')
            clip_text = self.text_encoder(clip_text)[0]
        else:
        ...
        if self.norm_embs:
            if self.hidden_state:
                # normalize all tokens by cls token's norm
                clip_text = clip_text / torch.norm(clip_text[:, 0], dim=-1).reshape(-1, 1, 1)
            else:
...
class BrainNetwork(nn.Module):
    def __init__(self, out_dim=768, in_dim=15724, clip_size=768, h=4096, n_blocks=4, norm_type='ln', act_first=False, use_projector=True):
        ...
        # act_fn = partial(nn.ReLU) if norm_type == 'bn' else nn.GELU
...
class BrainNetwork_p(nn.Module):
    def __init__(self, out_dim=768, in_dim=15724, clip_size=768, h=4096, n_blocks=4, norm_type='ln', act_first=False, use_projector=True):
        super().__init__()
        norm_func = partial(nn.BatchNorm1d, num_features=h) if norm_type == 'bn' else partial(nn.LayerNorm, normalized_shape=h)
        act_fn = partial(nn.ReLU, inplace=True) if norm_type == 'bn' else nn.GELU
    ...
    def forward(self, x):
        ...
        for res_block in range(self.n_blocks):
            ...
            residual = x
        x = x.reshape(len(x), -1)
        x = self.lin1(x.to(self.lin1.state_dict()['weight'].device)).to(residual.device)
        if self.use_projector:
            return x, self.projector(x.reshape(len(x), -1, self.clip_size))
        return x