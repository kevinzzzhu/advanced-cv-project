...
if subj == 1:
    ...
    model.voxelenc = VoxelEncoder(voxel_sizes=multi_subject_voxel_sizes, shared_embed_size=hidden_dim, hidden_size=hidden_dim, seq_len=seq_len)
else:
    model.voxelenc = VoxelEncoder(voxel_sizes=[num_voxels], shared_embed_size=hidden_dim, hidden_size=hidden_dim, seq_len=seq_len)
...
with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16):
    for batch in tqdm(range(0,len(test_unique_idx), minibatch_size)):
        ...
        scores.append(prior_out.flatten(1).detach().cpu().numpy())
