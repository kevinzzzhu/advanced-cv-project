...
def multi_topk(similarities,labels,k=5):
    if k > similarities.shape[1]:
        k = similarities.shape[1]
    topsum=0
    for i in range(k):
        topsum += torch.sum(torch.stack([torch.argsort(ss,axis=1)[:,-(i+1)] for ss in similarities]).mode(axis=0).values == labels)/len(labels)
    return topsum
...
def soft_cont_loss_ddp(student_preds, teacher_preds, teacher_aug_preds, temp=0.125, distributed=True):
    if not distributed:
        teacher_teacher_aug = (teacher_preds @ teacher_aug_preds.T)/temp
        teacher_teacher_aug_t = (teacher_aug_preds @ teacher_preds.T)/temp
        student_teacher_aug = (student_preds @ teacher_aug_preds.T)/temp
...
def soft_cont_loss_acc(student_preds, teacher_preds, teacher_aug_preds, accelerator, temp=0.125, distributed=True):
    if not distributed:
        teacher_teacher_aug = (teacher_preds @ teacher_aug_preds.T)/temp
        teacher_teacher_aug_t = (teacher_aug_preds @ teacher_preds.T)/temp
        student_teacher_aug = (student_preds @ teacher_aug_preds.T)/temp
        ...
        all_student_preds, all_teacher_preds, all_teacher_aug_preds = accelerator.gather_for_metrics((student_preds, teacher_preds, teacher_aug_preds))
