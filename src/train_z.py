...
def check_system_health():
    """Check system health to prevent crashes"""
    try:
        ...
        if gpus:
            ...
            if gpu_temp > 85:  # Critical temperature
                print(f"WARNING: GPU temperature is {gpu_temp}°C - stopping training")
                return False
        
        # Check memory usage
        memory = psutil.virtual_memory()
        if memory.percent > 95:  # Critical memory usage
            print(f"WARNING: Memory usage is {memory.percent}% - stopping training")
            return False
        ...
        if torch.cuda.is_available():
            ...
            print(f"GPU Memory: {gpu_memory:.1f}GB/{gpu_memory_total:.1f}GB ({gpu_memory_percent:.1%})")
            if gpu_memory_percent > 0.60:  # Further reduced to 60% for maximum safety
                print(f"WARNING: GPU memory usage is {gpu_memory:.1f}GB/{gpu_memory_total:.1f}GB ({gpu_memory_percent:.1%}) - stopping training")
                return False
...
BATCH_SIZE=14 
GRADIENT_ACCUMULATION_STEPS=3 # Accumulate gradients to maintain effective batch size (3*14=42)
...
# %%
# Load 73k NSD images - Keep on disk to save memory
f = h5py.File(f'{data_path}/coco_images_224_float16.hdf5', 'r')
images = f['images'] # Keep on disk, don't preload to CPU to save memory
...
print("Loaded all 73k possible NSD text_embeds to disk (not CPU)!", images.shape)
...
autoenc.enable_gradient_checkpointing()
...
for epoch in progress_bar:
    # Check system health before each epoch
    if not check_system_health():
        print("System health check failed - stopping training")
        break
    ...
    # Stream batches per iteration to avoid huge CPU allocations
    subj_iters = [iter(dl) for dl in train_dls]
    ...
    for train_i in loop:
        ...
        # with torch.cuda.amp.autocast(enabled=False):
            ...
            if train_i % GRADIENT_ACCUMULATION_STEPS == 0:
                optimizer.zero_grad()
            loss=0.

            # Build one combined batch from all subjects for this iteration
            image_batches = []
            voxel_batches = []
            perm_list = []
            betas_list = []
            ...
            for s_idx, s in enumerate(subj_list):
                try:
                    behav0, past_behav0, future_behav0, old_behav0 = next(subj_iters[s_idx])
                except StopIteration:
                    subj_iters[s_idx] = iter(train_dls[s_idx])
                    behav0, past_behav0, future_behav0, old_behav0 = next(subj_iters[s_idx])
                ...
                image_batches.append(image0)

                voxel0 = voxels[f'subj0{s}'][behav0[:,0,5].cpu().long()]
                ...
                if seq_len==1:
                    ...
                    if seq_past>0:
                        ...
                        past_voxel0 = voxels[f'subj0{s}'][past_behavior]
                        past_voxel0[past_behavior==-1] = voxel0[torch.where(past_behavior==-1)[0]]
                        past_voxel0 = torch.Tensor(past_voxel0)
                        for p in range(seq_past):
                            mask = (past_behav0[:,p,-1] == 1)
                    ...
                    if seq_future>0:
                        ...
                        future_voxel0 = voxels[f'subj0{s}'][future_behavior]
                        future_voxel0[future_behavior==-1] = voxel0[torch.where(future_behavior==-1)[0]]
                        future_voxel0 = torch.Tensor(future_voxel0)
                        for p in range(seq_future):
                            mask = (future_behav0[:,p,-1] == 1)
                            index = torch.nonzero(mask.cpu()).squeeze()
                ...
                voxel_batches.append(voxel0)

            # Concatenate across subjects
            image = torch.cat(image_batches, dim=0).to(device)
            voxel_list = [vb.detach().to(device) for vb in voxel_batches]
            ...
            if epoch < int(mixup_pct * num_epochs) and len(perm_list) > 0:
                perm = torch.cat([p.detach().to(device) for p in perm_list], dim=0)
                betas = torch.cat([b.detach().to(device) for b in betas_list], dim=0)
                select = torch.cat([sel.detach().to(device) for sel in select_list], dim=0)
            ...
            if use_cont:
                ...
                transformer_feats = None  # Initialize to None when not using cont
            ...
            if use_cont and transformer_feats is not None:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"Warning: NaN/Inf loss detected at epoch {epoch}, iteration {train_i}. Skipping this batch.")
                continue
            ...
            # Aggressive memory cleanup (after using variables)
            del voxel_list, image, image_512, image_enc, image_enc_pred
            if 'voxel_voxelenc' in locals():
                del voxel_voxelenc, voxel_voxelenc_list
            if 'transformer_feats' in locals() and transformer_feats is not None:
                del transformer_feats
            if 'image_enc_pred' in locals():
                del image_enc_pred
            if 'mse_loss' in locals():
                del mse_loss
            if 'cont_loss' in locals():
                del cont_loss
            torch.cuda.empty_cache()
    ...
    if local_rank==0:
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            ...
            for test_i, (behav, past_behav, future_behav, old_behav) in loop:  
                ...
                test_indices = torch.arange(len(test_voxel))[:100]  # Reduced from 300 to 100
                ...
                # Process in smaller chunks to avoid OOM
                image_enc = torch.cat([autoenc.encode(2*image_512[int(i*10):int((i+1)*10)]-1).latent_dist.mode() * 0.18215 for i in range(10)])  # Reduced from 30 to 10
                ...
                for rep in range(3):
                    ...
                    if use_cont:
                        ...
                        transformer_feats0 = None  # Initialize to None when not using cont
                    if rep==0:
                        image_enc_pred = image_enc_pred0
                        transformer_feats = transformer_feats0
                    else:
                        image_enc_pred += image_enc_pred0
                        if transformer_feats0 is not None:
                            transformer_feats += transformer_feats0
                image_enc_pred /= 3
                if transformer_feats is not None:
                    transformer_feats /= 3
